...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags) // all the shit
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool CustomSliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color, ImVec2 valuesize, const char* label, char* value)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    const ImGuiStyle& style = g.Style;

    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(frame_bb.Max.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImGui::GetColorU32(ImVec4(255, 255, 255, 200)));
    ...
    if (decimal_precision > 0)
        ...
        grab_sz = ImMin(ImMax(1.0f * (slider_sz / ((v_min < v_max ? v_max - v_min : v_min - v_max) + 0.5f)), style.GrabMinSize), slider_sz);
    const float slider_usable_sz = slider_sz - grab_sz;
    ...
    if (g.ActiveId == id)
    {
        if (g.IO.MouseDown[0])
        {
            ...
            new_value = ImGui::RoundScalar(new_value, decimal_precision);
            if (*v != new_value)
            {
                *v = new_value;
                value_changed = true;
    ...
    if (*v != v_min)
        window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(grab_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImGui::GetColorU32(ImVec4(255, 255, 255, 1.00f)));

    //Grab
    if (*v != v_min)
        window->DrawList->AddLine(ImVec2(grab_bb.Min.x + 10, grab_bb.Min.y + 5), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 4), ImGui::GetColorU32(g.ActiveId == id ? ImVec4(255, 255, 255, 1.00f)/*Active*/ : ImVec4(255, 255, 255, 1.00f)), 10.f);


    window->DrawList->AddText(ImVec2(frame_bb.Max.x - ImGui::CalcTextSize(value, label).x, frame_bb.Min.y + 0), ImColor(255, 255, 255), value, label);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() - 40;
    ...
    const char* value_buf_end2 = value_buf2 + ImFormatString(value_buf2, IM_ARRAYSIZE(value_buf2), display_format, *v);
    const bool value_changed = CustomSliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, 0, ImColor(255, 255, 255), CalcTextSize(value_buf, NULL, true), value_buf_end, value_buf);
    //const bool value_changed = CustomSliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, 0, ImColor(255, 255, 255), CalcTextSize(value_buf2, NULL, true), value_buf_end, value_buf2);


    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y - 7), label);
